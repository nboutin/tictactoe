...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4180)    // We attempt to stream a function (address) by const&, which
...
public:
    ...
    static IndexTracker&
    acquire(TrackerContext& ctx, NameAndLocation const& nameAndLocation, int size);
...
bool WithinUlpsMatcher::match(double const& matchee) const
{
    ...
    default: throw std::domain_error("Unknown FloatingPointKind value");
    }
}
...
IndexTracker&
IndexTracker::acquire(TrackerContext& ctx, NameAndLocation const& nameAndLocation, int size)
{
...
std::set<Verbosity> MultipleReporters::getSupportedVerbosities() { return std::set<Verbosity>{}; }
